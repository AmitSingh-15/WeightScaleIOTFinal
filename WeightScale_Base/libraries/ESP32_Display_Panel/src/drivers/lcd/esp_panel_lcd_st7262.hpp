...
public:
     ...
     */
    LCD_ST7262(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     * @note Vendor initialization commands may vary between manufacturers. Consult LCD supplier for them
     */
    LCD_ST7262(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_ST7262(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_ST7262(bus, 0, 0, color_bits, rst_io)
    {
    }
...
/**
 * @brief Alias for backward compatibility
 * @deprecated Use `esp_panel::drivers::LCD_ST7262` instead
 */
