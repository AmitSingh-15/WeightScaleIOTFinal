...
public:
     ...
     */
    LCD_NV3022B(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     * @note Vendor initialization commands may vary between manufacturers. Consult LCD supplier for them
     */
    LCD_NV3022B(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_NV3022B(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_NV3022B(bus, 0, 0, color_bits, rst_io)
    {
    }
...
/**
 * @brief Alias for backward compatibility
 * @deprecated Use `esp_panel::drivers::LCD_NV3022B` instead
 */
using ESP_PanelLcd_NV3022B [[deprecated("Use `esp_panel::drivers::LCD_NV3022B` instead")]] =
    esp_panel::drivers::LCD_NV3022B;
