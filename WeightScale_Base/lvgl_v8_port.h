 ...
 * The LCD must be configured with 3 frame buffers via configFrameBufferNumber(3)
 ...
 */
#define LVGL_PORT_BUFFER_MALLOC_CAPS            (MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)       // Allocate LVGL buffer in SRAM (DMA required for SPI/QSPI LCD)
// #define LVGL_PORT_BUFFER_MALLOC_CAPS            (MALLOC_CAP_SPIRAM)      // NOT recommended for SPI/QSPI LCD
#define LVGL_PORT_BUFFER_SIZE_HEIGHT            (2)
...
#define LVGL_PORT_TASK_STACK_SIZE               (8 * 1024)  // Headroom for 1024x600 full-refresh rendering
